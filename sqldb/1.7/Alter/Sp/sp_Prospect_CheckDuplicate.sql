...
ALTER PROCEDURE [dbo].[sp_Prospect_CheckDuplicate]      
...
BEGIN      
 ...
 BEGIN      
   ...
   where  ((ZipCode Is Not Null And ZipCode !='') and LEN(ZipCode)!=5 and LEN(ZipCode)!=9) or ISNUMERIC(ZipCode)!=1
   --AND ZipCode NOT LIKE '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]'
  ...
  where  PhoneNumber Is Not Null And PhoneNumber !='' and (LEN(PhoneNumber)!=10 or ISNUMERIC(PhoneNumber)!=1)--AND PhoneNumber NOT LIKE '[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]' 
  ...
  where  ProspectName Is Null OR ProspectName=''     
   ...
   where  ProspectName Is Not Null And ProspectName !='' and LEN(ProspectName)>200      
...
((((SUBSTRING(Imp.ProspectName, 1,10)= SUBSTRING(p.ProspectName, 1,10) OR Imp.ProspectName in (select p.ProspectName+' '+SpecialCare from ProspectNameSpecialCare))      
  And SUBSTRING(Imp.City, 1,20)=p.City And Imp.State=p.State) OR (  
  ((SUBSTRING(Imp.ProspectName, 1,10)= SUBSTRING(p.ProspectName, 1,10) OR Imp.ProspectName in (select p.ProspectName+' '+SpecialCare from ProspectNameSpecialCare))      
 ...
 LEFT JOIN Prospects P On (Pi.ProspectId = P.Id)      
  ...
  And SUBSTRING(Imp.City, 1,20)=cast(p.City as varchar(50)) And Imp.State=cast(p.State as varchar(50)))       
  OR       
  ((SUBSTRING(Imp.ProspectName, 1,10)= SUBSTRING(p.ProspectName, 1,10) OR Imp.ProspectName in (select p.ProspectName+' '+SpecialCare from ProspectNameSpecialCare))      
