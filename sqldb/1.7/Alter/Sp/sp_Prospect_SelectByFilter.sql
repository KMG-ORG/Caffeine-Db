...
AS                              
 BEGIN                    
  ...
  DECLARE @SQL NVARCHAR(Max) = 'SELECT *,COUNT(*) OVER() AS TotalRecords FROM (SELECT ' + Case When (( @PageSize Is Not Null and @PageSize>0) AND (@PageNumber IS NULL Or @PageNumber=0))  Then ('TOP ('+Cast(@PageSize as varchar(10))+')')    Else  ' ' END  
+                         
   ...
   (SELECT COUNT(*) from Disposition where ProspectId = Prospects.Id) as TotalDisposition,
