...
CREATE PROCEDURE [dbo].[sp_Disposition_Save]
( 
    @Id int OUTPUT,
	@DispositionId int,
	@ProspectId int,
...
BEGIN
	 ...
	 values
	 ('No Pitch Made - Talked to Non Decision Maker'),('Left Voice Message'),('Did Not Leave Message')
	   
	 UPDATE @TempNonFinalFieldTbl SET dispositionFieldId = (SELECT Id FROM MasterDisposition WHERE @DispositionId=dispositionField)
