...
-- =========================================================================================    
CREATE PROCEDURE [dbo].[sp_ManageList_Pagination]    
 @PageNumber int,      
 @PageSize int   ,
...
AS      
 ...
 SELECT 0 as Total,Cl.FilterQuery,Cl.Id,    
  ...
  Cl.TypeOfList,    
  Cl.ExpirationDate,     
  CASE WHEN Cl.IsNewList = 1 THEN 0 ELSE 1 END AS CreateFromTemplet,    
  0 AS CreatedTemplateId ,    
  '' as CreatedTemplateName,    
  0 SharedWithId,    
  '' As SharedWithName,     
 ...
 AND (@IsRecentDate = 0 Or (DATEDIFF(day, CreatedDateTime, GETDATE()) <=60))
